...
the {\em parameters\/} $\bbeta=(\beta_1,\beta_2,\ldots,\beta_P)\trans$
in the model
...
represents the {\em response\/} for {\em case\/} $n$, $n
=1,2,\ldots,N$, has a {\em deterministic\/} part and a {\em
  stochastic\/} part.  The deterministic part, $( x_{n1} ,\ldots,
x_{nP} ) \bbeta$, depends upon the parameters $\bbeta$ and upon the
{\em predictor\/} or {\em regressor variables\/} $x_{np}$,
$p=1,2,\ldots,P$.  The stochastic part, represented by the random
variable $Z_n$, is a {\em disturbance\/} which perturbs the response
...
\end{displaymath}
We therefore call $\bX \bbeta$ the {\em expectation function\/}
\index{expectation function}
for the regression model.
The matrix $\bX$ is called the {\em derivative matrix\/},
...
{\em derivatives with respect to any of the
...
for $\bY$, given  $\bbeta$ and the {\em variance\/}\index{variance}
$\sigma^2$, is
\begin{eqnarray}\label{eqn:1.4}
  p(\by|\bbeta,\sigma^2)&=&\left(2\pi\sigma^2\right)^{-N/2}\exp
  \left({ -\left( \by -\bX\bbeta\right)\trans\left(\by-\bX\bbeta\right)
...
The {\em likelihood function},\index{ likelihood function} 
or more simply, the {\em likelihood},
$l ( \bbeta , \sigma | \by )$,
...
\begin{equation}\label{eqn:1.5a}
  ...
  \left( { - \norm \by -\, \bX \bbeta \norm^2   \over  2 \sigma^2 }
  \right)
\end{equation}
The likelihood is maximized with respect to $\bbeta$ when the
{\em residual sum of squares}\index{residual "sum
...
\end{eqnarray}
is a minimum.
Thus the {\em maximum likelihood estimate\/}
\index{maximum likelihood estimate}
$\hat{\bbeta}$ is the value of $\bbeta$ which minimizes $S ( \bbeta )$.
This $\hat{\bbeta}$ is called the {\em least squares\/} estimate
...
\item A $1-\alpha$ {\em joint confidence region\/} for $\bbeta$
  ...
  where
  \begin{displaymath}
    s^2 =\  {S ( \hat{\bbeta})  \over N -P}    
  \end{displaymath}
  is the {\em residual mean square\/} or {\em variance estimate\/}
  \index{residual "mean square"}
  \index{ variance estimate}
  based on $N -P$ {\em degrees of freedom\/},
  \index{ "degrees of freedom"}
  and $\FPNP$ is the upper $\alpha$
  quantile for Fisher's F distribution with $P\/$ and $N-P\/$
  \index{ "F distribution"}
  degrees of freedom.
\item A $1-\alpha$ {\em marginal confidence interval\/} for the 
...
\index{likelihood!region}
  \begin{displaymath}
    c=\ S( \hat{\bbeta}) \left[1+{P\over N-P}\FPNP\right]    
  \end{displaymath}
...
is
  \begin{equation}\label{eqn:1.13}
    ...
    {1 +\, {( \bbeta \,-\, \hat{\bbeta}) \trans \bX \trans \bX
    ( \bbeta -\, \hat{\bbeta})   \over  \nu s^2 }}\right\}^{ - ( \nu + P )/2}
  \end{equation}
which is in the form of a $P\/$-variate Student's T density with
\index{T distribution!multivariate}
{\em location parameter\/} $\hat{\bbeta}$, {\em scaling matrix\/}
$ s^2 ( \bX \trans \bX )^{-1} $,
...
A {\em highest posterior density\/} (HPD) region of content $1-\alpha$
...
use of the term {\em spherical normal\/}
...
concerned with how the expected response depends on the {\em
parameters}, not with how it depends on the {\em variables}.
...
involves two steps:
  \begin{enumerate}
    \item Using the $P\/$-dimensional parameter vector $\bbeta$ and the
          ...
          $N\/$-dimensional
          {\em expected response vector\/}\index{ "expected response" vector}
...
$P\/$-dimensional {\em expectation surface\/} in the $N\/$-dimensional
response space. 
\index{response!space}
This surface is a linear subspace of the response space, so we call it
the {\em expectation plane\/}\index{ expectation plane} when dealing
with a linear model.
...
The angle $\omega$ between them can be calculated from
  \begin{eqnarray*}
    \cos\omega&=&{\bx_1\trans\bx_2\over\norm\bx_1\norm\norm\bx_2\norm}\\
    &=&{5.30\over\sqrt{(3)(9.83)}}\\
    &=&0.98
  \end{eqnarray*}
...
The area of the parallelogram is
  ...
  \end{eqnarray}
That is, the {\em Jacobian determinant\/} of the transformation
...
be {\em orthogonal}, or {\em normal}, to the expectation plane.
...
satisfies the {\em normal equations}
...
of $\bQ$, we have
  \begin{equation}\label{eqn:qrq1r1}
    \bX =\bQ\bR=\bQ_1 \bR_1
  \end{equation}
Performing a {\em QR\/} decomposition is straightforward, as is shown in
Appendix 2.

Geometrically, the columns of $\bQ$ define an {\em orthonormal}, or
{\em orthogonal}, basis
...
As shown in Appendix 2, the {\em QR\/} decomposition (\ref{eqn:qrq1r1})
of the matrix
  \begin{displaymath}
    \bX =
    \begin{bmatrix}
...
$\biZ = (Z_1 ,\ldots, Z_n ) \trans $ is
    \begin{displaymath}
      p_{\biZ} ( \bz ) =
( 2 \pi \sigma^2 )^{-N/2} \exp \left( { - \bz \trans \bz  
\over  2 \sigma^2 }\right)
    \end{displaymath}
...
\begin{displaymath}
  p_{\biU} ( \bu )   =
  (2 \pi \sigma^2 )^{-N/2} | \bQ |
  \exp \left( {- \bu \trans \bQ \trans \bQ \bu  \over  2 \sigma^2 }\right)
\end{displaymath}
\begin{displaymath}
  = ( 2 \pi \sigma^2 )^{-N/2}
  \exp \left( {- \bu \trans \bu  \over  2 \sigma^2 }\right)
\end{displaymath}
...
Figure \ref{fig:PCBpartialht} and calculate the ratio
    \begin{eqnarray*}
      {\left( u_2^0 \right)^2   \over  s^2 }&=&{0.240 \over 0.246 }\\
       &=&0.97
    \end{eqnarray*}
...
The quantity
    ...
    \end{equation}
is called the {\em standard error\/}
\index{standard error}
...
%\glossary{ Ansley, C.F.}
linear regression using the {\em QR\/} decomposition, which, as we have seen,
...
Since we cannot guarantee {\em a priori\/} that the different
...
data using that model, then go back and use {\em diagnostics\/}
\index{diagnostics}
...
\begin{enumerate}
\item {\em The expectation function is correct}.
  ...
  and that it does {\em not\/} include any unimportant predictor
  variables.
  ...
  are done,
  \index{ randomization}
  to ensure that {\em causation}, not {\em correlation},
  is being determined \cite{box:1960}.
                                %\glossary{ Box, G.E.P.}
\item {\em The response is expectation function plus
    disturbance}.
...
\item {\em The disturbance is independent of the expectation
    function}.
  ...
  assumption implies that {\em any important variables which
...
\item {\em Each disturbance has a normal distribution}.
  \index{ normal distribution}
  ...
  squares estimates
                                %\glossary{ Seber, G.A.}
  are the best {\em both individually and in any linear
  ...
  \index{ assumptions "normal disturbances"}
\item {\em Each disturbance has zero mean}.
...
\item {\em The disturbances have equal variances}.
  \index{ variance constant}
  \index{ assumptions "constant variance"}
  \index{ disturbance "constant variance"}
  ...
  values are {\em equally unreliable}, and so the simple
  ...
  averages and variances to determine a suitable {\em
    variance-stabilizing\/}\index{ variance "-stabilizing
  ...
  the normal density.
\item {\em The disturbances are distributed independently}.
...
is to plot the {\em studentized residuals},
...
is appropriate, such a {\em normal probability plot\/} of the residuals
...
perform tests for {\em lack of fit\/} of the expectation function.
...
of freedom is decomposed into the {\em replication\/} sum of squares
...
say, $\nu_r$ degrees of freedom, and the {\em lack of fit\/} sum of
...
\begin{problems}
  ...
  \prob
  \subprob
    Fit the model $\ln(\mbox{\rm PCB})=\beta_1+\beta_2\mbox{\rm age}$
